...
#define ENCLAVE_PATH "libenclave1.so"
...
bool get_app_enclave_report(sgx_target_info_t qe_target_info, sgx_report_t *app_report, sgx_enclave_id_t eid)
{
    ...
    sgx_status = Enclave1_enclave_create_report(eid, &retval, &qe_target_info, app_report);
...
int quoteVerify(string quote_path, sgx_enclave_id_t eid)
{
    ...
    sgx_ret = Enclave1_enclave_verify_quote(eid, &retval, &measurement, &ret);      // 调用 ecall 函数
...
int main()
{
    ...
    if (quoteVerify(quote_path, eid)) {
        printf("Quote verified successfully!\n");
    }
    else {
        printf("Quote verified failed!\n");
    }
    return 0;
}
